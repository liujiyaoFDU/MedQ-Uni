...
class InterleaveInferencer:
    ...
    def _calculate_target_size_with_aspect_ratio(self, original_width, original_height):
        """
        DEPRECATED: 此方法已废弃，请使用 get_actual_transformed_size() 代替

        Calculate target image size preserving aspect ratio with VAE transform constraints.

        警告：此方法的计算结果可能与实际transform结果不一致，导致尺寸错位。
        推荐使用 get_actual_transformed_size() 直接获取transform后的实际尺寸，
        确保100%准确的尺寸对齐。

        此方法保留仅为向后兼容，inferencer内部的VQA重建方法仍在使用。
        """
    ...
    def get_actual_transformed_size(self, image):
        """
        获取图像经过VAE transform后的实际尺寸

        此方法直接执行transform操作并返回实际尺寸，确保与真实处理结果100%一致。
        推荐在需要精确尺寸对齐的场景中使用此方法，而非_calculate_target_size_with_aspect_ratio()。

        Args:
            image: PIL.Image 对象

        Returns:
            tuple: (height, width) 经过transform后的实际尺寸
        """
        # 直接transform图像获取实际tensor
