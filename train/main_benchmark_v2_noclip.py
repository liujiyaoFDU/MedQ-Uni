...
"""
Unified NAVIT Training Script - Benchmark Version

This is a modified version of main.py specifically for benchmarking experiments.
It includes additional performance monitoring:
- Real-time GPU memory tracking
- Sec/step metric calculation
...
- BENCHMARK: Added GPU memory monitoring and CSV logging
"""
...
def main():
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        # V2: Gradient clipping disabled to allow full gradient effect
        # total_norm = fsdp_model.clip_grad_norm_(training_args.max_grad_norm)
        # clipped_ratio = total_norm.item() / training_args.max_grad_norm if training_args.max_grad_norm > 0 else 0.0
        ...
        if curr_step % training_args.log_every == 0:
            ...
            peak_memory_allocated = torch.cuda.max_memory_allocated(device) / (1024**3)  # Convert to GB (实际使用)
            peak_memory_reserved = torch.cuda.max_memory_reserved(device) / (1024**3)    # Convert to GB (向GPU申请的总量)
            torch.cuda.reset_peak_memory_stats(device)
            ...
            # V2: Gradient clipping disabled, no total_norm/clipped_ratio to log
            # log['total_norm'] = total_norm.item()
            # log['clipped_ratio'] = clipped_ratio
            ...
            if dist.get_rank() == 0:
                ...
                try:
                    with open(csv_path, 'a') as f:
                        if write_header:
                            f.write("step,ce_loss,mse_loss,weighted_ce_loss,weighted_mse_loss,total_loss,total_norm,clipped_ratio,sec_per_step,peak_memory_allocated_gb,peak_memory_reserved_gb,lr\n")
                        ...
                        total_norm_val = log.get('total_norm', 0.0)
                        clipped_ratio_val = log.get('clipped_ratio', 0.0)
                        lr_val = log['lr']
                        f.write(f"{curr_step},{ce_loss_val:.6f},{mse_loss_val:.6f},"
                                f"{weighted_ce_val:.6f},{weighted_mse_val:.6f},{total_loss_val:.6f},"
                                f"{total_norm_val:.6f},{clipped_ratio_val:.6f},"
