...
def main():
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        if curr_step % training_args.log_every == 0:
            ...
            # 追加加权后的分项损失，便于观察“实际参与总损失”的数值
            weighted_terms = {}
            if "ce" in log and hasattr(training_args, "ce_weight"):
                weighted_terms["ce_weighted"] = log["ce"] * training_args.ce_weight
            if "mse" in log and hasattr(training_args, "mse_weight"):
                weighted_terms["mse_weighted"] = log["mse"] * training_args.mse_weight
            if "pixel" in log and hasattr(training_args, "pixel_loss_weight"):
