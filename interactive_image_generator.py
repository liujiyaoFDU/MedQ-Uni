...
DEFAULT_CONFIG = {
    ...
    "model_path": "/inspire/hdd/global_user/hejunjun-24017/junzhin/projects/Uni-MedVL/output/ixi_t1_medq_1ep_ver1/0000600",
    "target_gpu_device": "0",
...
class ImageGenerator:
    ...
    def load_model(self):
        ...
        # Tokenizer
        # Load tokenizer from model_path (checkpoint directory)
        # The checkpoint directory should contain: vocab.json, merges.txt, tokenizer_config.json, tokenizer.json
...
image_path = "/inspire/hdd/global_user/hejunjun-24017/junzhin/data/bagel/annotation_medq-Uni/images/IXI-T1/process/high_res/IXI322-IOP-0891-T1/020_motion.png"
edit_instruction = """
As an image quality optimizer, enhance this medical image to improve visual quality while maintaining anatomical consistency and structural integrity. Please address the motion artifacts present in the input image.
"""
...
if generator.loaded and os.path.exists(image_path):
    ...
    # Image editing - 使用实际transform后的尺寸确保100%对齐
    actual_size = generator.inferencer.get_actual_transformed_size(input_image)
    ...
    if edited_image:
        ...
        image_output_path = os.path.join(output_dir, f"edited_image_{timestamp}.png")
        edited_image.save(image_output_path)
        print(f"Edited image saved to: {image_output_path}")
        ...
        # Save metadata and text results
        metadata_path = os.path.join(output_dir, f"metadata_{timestamp}.txt")
        with open(metadata_path, 'w', encoding='utf-8') as f:
            f.write("=== Image Editing Metadata ===\n")
            f.write(f"Timestamp: {timestamp}\n")
            f.write(f"Input image: {image_path}\n")
            f.write(f"Input size: {input_image.size}\n")
            f.write(f"Target size: {target_size}\n")
            f.write(f"\n=== Edit Instruction ===\n{edit_instruction}\n")

            if use_thinking and thinking_result:
                analysis_text = thinking_result.get('text', '')
                f.write(f"\n=== Thinking Mode Analysis ===\n{analysis_text}\n")
            elif understanding_result:
                understanding_text = understanding_result.get('text', '')
                f.write(f"\n=== Understanding Mode Context ===\n{understanding_text}\n")

            f.write(f"\n=== Generation Parameters ===\n")
            f.write(f"cfg_text_scale: {cfg_text_scale}\n")
            f.write(f"cfg_img_scale: {cfg_img_scale}\n")
            f.write(f"num_timesteps: {num_timesteps}\n")
            f.write(f"timestep_shift: {timestep_shift}\n")
            f.write(f"seed: {seed}\n")
