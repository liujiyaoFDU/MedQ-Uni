...
"""
Unified NAVIT Training Script - Memory Efficient Version

基于 main_benchmark.py，增强了参数管理和显存优化功能：
- 精确的MOT分支级别参数冻结（freeze_mot_und/freeze_mot_gen）
- 改进的优化器参数组创建（只添加requires_grad=True的参数）
- 详细的参数统计和验证
- 显存占用监控

Key Features:
- MOT分支独立控制：可单独训练Understanding或Generation专家
- 参数完整性验证：确保冻结参数不进入优化器
- 详细统计输出：区分总参数、可训练参数、优化器管理的参数
- 内存优化：避免为冻结参数分配优化器状态
"""
...
def main():
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        if curr_step % training_args.log_every == 0:
            ...
            log['training_mode'] = 'memory_efficient'
