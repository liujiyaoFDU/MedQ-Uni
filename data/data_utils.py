...
def prepare_attention_mask_per_sample(split_lens, attn_modes, device="cpu"):
    ...
    for s, attn_mode in zip(split_lens, attn_modes):
        ...
        if attn_mode == "causal":
            attention_mask[csum:csum + s, csum:csum + s] = torch.ones((s, s), dtype=torch.bool, device=device).tril()
            attention_mask[csum:csum + s, :csum] = 1
        else:
            attention_mask[csum:csum + s, csum:csum + s] = torch.ones((s, s), dtype=torch.bool, device=device)
            attention_mask[csum:csum + s, :csum] = 1
        csum += s
    ...
    for s, attn_mode in zip(split_lens, attn_modes):
        if attn_mode == "noise":
            attention_mask[:, csum : csum + s] = torch.zeros((sample_len, s), dtype=torch.bool, device=device)
            attention_mask[csum : csum + s, csum : csum + s] = torch.ones((s, s), dtype=torch.bool, device=device)
        csum += s
